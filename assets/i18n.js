...
(function () {
  ...
  var DICT = {
    ...
    /* ---------- lote 24: revisao das tres paginas de futebol ---------- */
    "Segunda Guerra": "Second World War",
    "(sem Copas em 1942/46)": "(no World Cups in 1942/46)",
    "atropelou": "thrashed",
    "Placar": "Scoreline",
    "Ocorrências": "Times",
    ...
    "bloqueado": "blocked",
    "na trave": "off the post",
    "Bloco defensivo (360)": "Defensive block (360)",
    "defende ←": "defending ←",
    "posições reais dos defensores durante passes adversários": "real positions of the defenders during opposition passes",
    "clique p/ freeze frame": "click for the freeze frame",
    "clique para ver o freeze frame": "click to see the freeze frame",
    "clean sheets": "clean sheets",
    "assist.": "assists",

    /* ---------- lote 24b: meses no DICT (para grupos de captura) ---------- */
    "janeiro": "January",
    "fevereiro": "February",
    "março": "March",
    "abril": "April",
    ...
    /* ---------- lote 24d: fragmento sem o numero ---------- */
    "jogadores disputaram": "players played",
    "jogador disputou": "player played",

    /* ---------- lote 25: tags e rotulos dos cards de fontes ---------- */
    "Pontos fortes": "Strengths",
    "Pontos fracos": "Weaknesses",
    "1200+ ligas": "1,200+ leagues",
    "API free 100/dia": "Free API, 100/day",
    "API moderna e documentada": "A modern, documented API",
    "Cobre WC 2026 e grandes ligas": "Covers the 2026 World Cup and the major leagues",
    "Elencos": "Squads",
    "Free = 100 req/dia + temporadas atuais/2026 bloqueadas": "Free = 100 requests/day, and the current/2026 seasons are locked",
    "Free tier muito limitado": "Very limited free tier",
    "Inclui odds e predictions": "Includes odds and predictions",
    "Live scores e tabelas": "Live scores and tables",
    "Ratings detalhados": "Detailed ratings",
    "Schema documentado, ecossistema (statsbombpy, Python/R)": "Documented schema and an ecosystem (statsbombpy, Python/R)",
    "Stats agregadas": "Aggregated stats",
    "Temporada 17/18": "2017/18 season",
    "xG agregado": "Aggregated xG",

    /* ---------- lote 25b: cabecalhos dos cards ---------- */
    "Granularidade": "Granularity",
    "Acesso": "Access",
    "Granularidade agregada (sem evento/coords)": "Aggregated granularity (no events or coordinates)",
    "Orientado a apostas, não a tracking": "Built for betting, not for tracking",
    "Granularidade menor que Sofascore/FotMob.": "Lower granularity than Sofascore/FotMob.",
  ...
  var RULES = [
    ...
    [/^jogadores de Copa nasceram em (\d+) de (.+):$/, "World Cup players were born on $1 $2:"],
    [/^(\d+) envolvimentos em passes completos$/, "$1 involvements in completed passes"],
    [/^(.+) envolvimentos em passes completos$/, "$1 involvements in completed passes"],
    [/^(.+) jogos · (.+) defesas · (.+) gols sofridos$/, "$1 matches · $2 saves · $3 goals conceded"],
    [/^Mostrando todas as (.+) convoca\u00e7\u00f5es\.$/, 'Showing all $1 call-ups.'],
    [/^venceu (.+)% \u00b7 empatou (.+)% \u00b7 perdeu (.+)%$/, "won $1% · drew $2% · lost $3%"],
    [/^jogador de Copa nasceu em (\d+) de (.+):$/, "World Cup player was born on $1 $2:"],
    [/^\((.+)\), com (.+) torcedores acumulados\.$/, "($1), with $2 fans in total."],
    ...
    [/^nenhuma disputa de pênaltis$/, "no penalty shoot-outs"],
    [/^(.+) chutes \(p\u00eanaltis ocultos\)$/, "$1 shots (penalties hidden)"],
    [/^(.+) gols marcados · (.+) sofridos$/, "$1 goals scored · $2 conceded"],
    ...
    [/^(.+) km por 90 min · (.+) sprints$/, "$1 km per 90 min · $2 sprints"],
    [/^aproveitamento: (.+)% no alvo$/, "accuracy: $1% on target"],
    ...
    [/^(.+) gols · (.+) assist\. · (.+) min$/, "$1 goals · $2 assists · $3 min"],
    [/^Mandante (.+) × (.+) Visitante$/, "Home $1 × $2 Away"],
    [/^(.+) jogos com prorrogação$/, "$1 matches that went to extra time"],
    [/^(.+) \(mil R\$, escala log\)$/, '$1 (R$ thousand, log scale)'],
    [/^(.+) disputas de pênaltis$/, "$1 penalty shoot-outs"],
    [/^(.+) jogadores disputaram$/, "$1 players played"],
    [/^(.+) gols · xG (.+) · (.+) chutes$/, "$1 goals · xG $2 · $3 shots"],
    [/^posse (.+)% · (.+) chutes\/90$/, "possession $1% · $2 shots/90"],
    [/^diferen\u00e7a de (.+) pontos$/, "a gap of $1 points"],
    [/^(.+) atuações de 60\+ min$/, "$1 appearances of 60+ min"],
    [/^Jogador (\d+) → Jogador (\d+):$/, "Player $1 → Player $2:"],
    [/^(.+) disputa de pênaltis$/, "$1 penalty shoot-out"],
    [/^(.+) municípios . total (.+)$/, '$1 municipalities - total $2'],
    [/^Líder em Produção: (.+)$/, 'Production leader: $1'],
    [/^(.+)% contra (.+)% da m\u00e9dia$/, "$1% against the average of $2%"],
    [/^(.+)% de todos os jogos$/, "$1% of all matches"],
    [/^(.+) gols\/90 · (.+) chutes$/, "$1 goals/90 · $2 shots"],
    ...
    [/^(.+) sprints por jogo$/, "$1 sprints per match"],
    [/^(.+) jogos no torneio$/, "$1 matches in the tournament"],
    [/^média do torneio (.+)$/, "tournament average $1"],
    [/^(\d+) seleções · (\d+) gols$/, "$1 teams · $2 goals"],
    ...
    [/^(.+) km por 90 min$/, "$1 km per 90 min"],
    [/^pico de (.+) km\/h$/, "peak of $1 km/h"],
    [/^(.+) gols por jogo$/, "$1 goals per match"],
    [/^(.+) jogadores \((.+)%\)$/, "$1 players ($2%)"],
    [/^Mostrar mais (.+)$/, "Show $1 more"],
    [/^(.+) nos p\u00eanaltis$/, "$1 on penalties"],
    [/^(.+) \(escala log\)$/, '$1 (log scale)'],
    [/^(.+) \(escala log\)$/, "$1 (log scale)"],
    [/^(.+) m por 90 min$/, "$1 m per 90 min"],
    [/^(.+) km em (.+) jogos$/, "$1 km in $2 matches"],
    [/^★★★★★ 5 COPAS$/, "★★★★★ 5 WORLD CUPS"],
    [/^(.+) convoca\u00e7\u00f5es$/, '$1 call-ups'],
    [/^(.+) - (.+) - Global$/, '$1 — $2 — Global'],
    [/^(.+) km por jogo$/, "$1 km per match"],
    [/^(.+) do torneio (.+)$/, "$1 of the tournament $2"],
    [/^(.+) vs esperado$/, "$1 vs expected"],
    [/^(.+) municípios$/, '$1 municipalities'],
    [/^(.+) habitantes$/, '$1 inhabitants'],
    [/^Líder em (.+): (.+)$/, '$1 leader: $2'],
    [/^lota\u00e7\u00e3o em (.+)$/, "posted in $1"],
    [/^(.+) jogadores\.$/, "$1 players."],
    [/^(.+) gols\/jogo$/, "$1 goals/match"],
    [/^mostrando (.+)$/, 'showing $1'],
